...
# The following steps must be completed prior to running this code:
#> 1. Download eBird species data 
#> 2. Download eBird Sampling data
...
# file to save spatial data - change to whatever we want 
gpkg_dir <- "data-ghb"
...
# Get land border with lakes removed
ne_land <- ne_download(scale = 50, category = "cultural",
                       type = "admin_0_countries_lakes",
                       returnclass = "sf") %>%
  # NOTE: For whole world just remove this line (but increases MODIS used)
...
# Filter to Asia with st_intersect()
...
# NOTE: for single country don't need country lines
...
ebd <- auk_ebd("/Users/hamishcampbell/Documents/Cambridge/Summer/HuntingPressure/data_analysis/eBird/data-ghb/ebd_IN_grehor1_relFeb-2022.txt", 
               file_sampling = "/Users/hamishcampbell/Documents/Cambridge/Summer/HuntingPressure/data_analysis/eBird/data-ghb/ebd_sampling_relFeb-2022.txt")
...
ebd_filters <- ebd %>% 
  ...
  auk_species("Great Hornbill") %>%
  # Restrict checklists to just India
  auk_country("India") %>%
  # Only keep data collected in June, use * to get data from any year
...
data_dir <- "data-ghb/"
f_ebd <- file.path(data_dir, "ebd_ghb_india.txt")
f_sampling <- file.path(data_dir, "ebd_checklists_india.txt")
...
write_csv(ebird, "data-ghb/ebd_ghb_india_zf.csv", na = "")
...
legend("bottomleft", bty = "n",
       col = c("#555555", "#4daf4a"),
       legend = c("eBird checklists", "Great Hornbill sightings"),
       pch = 19)
box()
title("Great Hornbill eBird Observations\n 2010-2019, India")
...
india <- read_sf("data-ghb/gis-data.gpkg", "ne_land") %>% 
...
# then save in subfolder 'MODIS'
tifs <- runGdal(product = "MCD12Q1", collection = "006", SDSstring = "01", 
                extent = india %>% st_buffer(dist = 10000), 
                begin = begin_year, end = end_year, 
                outDirPath = "data-ghb", job = "modis",
...
landcover <- list.files("data-ghb/modis", "^modis_mcd12q1_umd", 
                        full.names = TRUE) %>% 
  stack()
...
# Save landcover checklist data
write_csv(pland, "data-ghb/modis_pland_location-year.csv")
...
r <- writeRaster(r, filename = "data-ghb/prediction-surface.tif", overwrite = TRUE)
...
forest_cover <- pland_coords %>% 
  ...
  # rasterize points
  rasterize(r, field = "pland_02_evergreen_broadleaf") %>% 
...
t <- str_glue("Proportion of Evergreen Broadleaf Forest\n",
...
elev <- raster("data-ghb/elevation_1KMmd_GMTEDmd.tif")
...
write_csv(pland_elev_checklist, "data-ghb/pland-elev_location-year.csv")
...
write_csv(pland_elev_pred, "data-ghb/pland-elev_prediction-surface.csv")
